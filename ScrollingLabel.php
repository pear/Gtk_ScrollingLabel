 ...
 */
define('GTK_SCROLL_LEFT',  1);
/**
 * A constant for representing srolls that move from right to left
 * @const GTK_SCROLL_RIGHT
 */
define('GTK_SCROLL_RIGHT', 2);
/**
 * A SrollingLabel error code.
 * @const GTK_SCROLL_DEFAULT_ERROR
 */
define('GTK_SCROLL_DEFAULT_ERROR', 1);
...
class Gtk_ScrollingLabel {
  ...
  var $speed = 70;
  /**
   * The first character to show when direction is GTK_SCROLL_LEFT
   * The last character to show when direction is GTK_SCROLL_RIGHT
   * @var integer
   */
  ...
  function _scrollLabel()
  {
    ...
    if ($this->direction == GTK_SCROLL_LEFT) {
	...
	$this->setDirection(GTK_SCROLL_LEFT);
   ...
   * is scrolling left to right (GTK_SCROLL_RIGHT), the 
   ...
   * right to left (GTK_SCROLL_LEFT), the current character
  ...
  function unPause()
  {
    ...
    if (isset($this->gtkTimeoutTag)) {
      ...
      $this->_error('Cannot unpause. Text not scrolling.', GTK_SCROLL_DEFAULT_ERROR, PEAR_ERROR_PRINT);
      return false;
  ...
  function pause()
  {
    ...
    if (empty ($this->gtkTimeoutTag)) {
      ...
      $this->_error('Cannot pause. No timeout found.', 
		    GTK_SCROLL_DEFAULT_ERROR, PEAR_ERROR_PRINT);
      return false;
   ...
   * constants GTK_SCROLL_LEFT and GTK_SCROLL_RIGHT which move the
   * from right to left and left to right respectively. The return
   * value is the diretion the text will move on the next call to 
   * _scrollLabel().
   *
   * @access public
   * @param  integer $direction (GTK_SCROLL_LEFT|GTK_SCROLL_RIGHT)
   * @return integer
   */
  function setDirection($direction)
  {
    // Check direction.
    if ($direction != GTK_SCROLL_LEFT && $direction != GTK_SCROLL_RIGHT) {
   ...
   */
  function _error($msg, $code = GTK_SCROLL_DEFAULT_ERROR, $pearMode = PEAR_ERROR_RETURN)
  {    
    ...
    if ($pearMode == PEAR_ERROR_PRINT) {
      PEAR::setErrorHandling(PEAR_ERROR_PRINT, "ScrollingLabel error: %s<BR />\n");
    }
